...
def EcritNombreDecimal(n):
    ...
    if not d:
        ...
        txt = NombreEnLettres(e)
    if d:
        partieDec = [_(u" dixième"), _(u" centième"), _(u" millième")]
        ...
        # le texte se finit par un. On l'accorde en genre avec unité
            ...
            txt = txt + "e"
        if e == 1:
            txt = txt + _(u' unité et ')
        if e > 1:
            txt = txt + _(u' unités et ')
...
valeurs = [_(u"milliers"), _(u"centaines"), _(u"dizaines"), _(u"unités"),
...
def _ClasserNombres():
    exo = ["\\exercice", '\\begin{enumerate}']
    cor = ["\\exercice*", '\\begin{enumerate}']
    classer(exo, cor, _("croissant"))
    classer(exo, cor, _("décroissant"))
    exo.append('\\end{enumerate}')
    cor.append('\\end{enumerate}')
    return (exo, cor)
