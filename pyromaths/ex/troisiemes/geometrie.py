...
def tex_enonce_thales(noms, valeurs, text_arrondi):
    ...
    texte = \
        _(u'Sur la figure ci-contre, les droites $(%s)\\text{ et }(%s)$ sont parallèles.\\par\n') % \
        (lAB(noms[1:3]), lAB(noms[3:5]))
    liste = long_val(noms, valeurs)
    texte += \
        _('On donne $%s=\\unit[%s]{cm}$ \\quad $%s=\\unit[%s]{cm}$ \\quad $%s=\\unit[%s]{cm}$ \\quad $%s~=~\\unit[%s]{cm}$.\\par\n') % \
        tuple(liste[0:8])
    texte += _('Calculer $%s$ et $%s$, ') % tuple(liste[8:10])
...
def tex_resolution_thales3(n, v, arrondi):
    ...
    texte = \
        _('$\\cfrac{%s}{%s}=\\cfrac{%s}{%s}\\quad$ donc $\\boxed{%s=\\cfrac{%s\\times %s}{%s}%s}$\\hfill') % \
        tuple(donnees[0:9])
    texte = texte + \
            _('$\\cfrac{%s}{%s}=\\cfrac{%s}{%s}\\quad$ donc $\\boxed{%s=\\cfrac{%s\\times %s}{%s}%s}$\\par\n') % \
            tuple(donnees[9:18])
    return texte
...
def fig_thales(noms, valeurs):
    ...
    if type_thales == 1:
        return (echelle, mini_x, mini_y, maxi_x, maxi_y, 225, angle + 45, noms[0], noms[1], noms[2], v[0], v[1], angle,
                - 45, angle + 90, noms[3], noms[4], v[3], v[4], angle,)
    else:
        return (echelle, mini_x, mini_y, maxi_x, maxi_y, 135, angle + 45, noms[0], noms[1], noms[2], v[0], v[1], angle,
                135, angle + 180, noms[3], noms[4], - v[3], - v[4], angle,)
