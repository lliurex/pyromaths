...
def tex_trouve_diviseur(a):  # trouve si les nombres dans le tuple a sont divisible par 10, 2, 5, 9 ou 3 (dans cet ordre)
    if a[0] % 10 == 0 and a[1] % 10 == 0:
        ...
        return _(u'La somme des chiffres de \\nombre{') + str(a[0]) + \
            _('} et celle de \\nombre{') + str(a[1]) + \
            _('} sont divisibles par neuf donc ils sont divisibles par 9.\\par\n') + \
            '\\nombre{' + str(a[0]) + _('} et \\nombre{') + str(a[1]) + \
            _('} ne sont donc pas premiers entre eux')
    elif a[0] % 3 == 0 and a[1] % 3 == 0:
        return _('La somme des chiffres de \\nombre{') + str(a[0]) + \
            _('} et celle de \\nombre{') + str(a[1]) + \
            _('} sont divisibles par trois donc ils sont divisibles par 3.\\par\n') + \
            '\\nombre{' + str(a[0]) + _('} et \\nombre{') + str(a[1]) + \
            _('} ne sont donc pas premiers entre eux')
...
def tex_algo_euclide(l):  # renvoie l'ecriture au format tex de l'algorithme d'Euclide
    ...
    for i in range(len(l)):
        lignes.append('\\nombre{%s}=\\nombre{%s}\\times\\nombre{%s}+\\nombre{%s}' % 
                      l[i])
    lignes.append(_('\\fbox{Donc le \\textsc{pgcd} de \\nombre{%s} et \\nombre{%s} est %s}.\n') % 
